...
#################################################################################################################
# Based on Mistral reference implementation
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if positions.shape[0] > 1:
            # prefill
            key, value = self.cache_k[:bsz], self.cache_v[:bsz]
        else:
            cur_pos = positions[-1].item() + 1
            key, value = self.cache_k[:bsz, :cur_pos, ...], self.cache_v[:bsz, :cur_pos, ...]
        ...
        scores = torch.matmul(xq, key.transpose(2, 3)) * self.scale
        if mask is not None:
