...
def run_ministral_demo(user_input, batch_size, device, instruct_mode, is_ci_env, num_batches, print_to_file, is_n300):
    # Create batch output file
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_directory = "models/demos/wormhole/ministral8b/demo/output"
    os.makedirs(output_directory, exist_ok=True)
    os.chmod(output_directory, 0o755)
    output_filename = f"{output_directory}/demo_user_output_{timestamp}.txt"

    # Set Ministral flags for CI
    if is_ci_env and instruct_mode:  # Update paths for instruct mode, otherwise use default paths for general weights
        os.environ["MINISTRAL_CKPT_DIR"] = "/mnt/MLPerf/tt_dnn-models/Mistral/ministral-8b-instruct-2410/"
        os.environ["MINISTRAL_TOKENIZER_PATH"] = "/mnt/MLPerf/tt_dnn-models/Mistral/ministral-8b-instruct-2410/"
        os.environ["MINISTRAL_CACHE_PATH"] = "/mnt/MLPerf/tt_dnn-models/Mistral/ministral-8b-instruct-2410/"
...
def load_model_and_tokenizer(device_id=0, batch_size=1, max_seq_len=512, instruct=True):
    """
    Load model and tokenizer for server use.
    ...
    try:
        ...
        model = TtTransformer(
            ...
            dtype=ttnn.bfloat8_b,
            state_dict=state_dict,
            weight_cache_path=model_args.weight_cache_path,
        )
...
def process_question(question, batch_size=1, max_seq_len=512, device_id=0, instruct=True, temperature=0.7):
    ...
    try:
        ...
        # Use existing model and tokenizer if available from server's global state
        import sys
        server_module = sys.modules.get('__main__')
        if server_module and hasattr(server_module, 'MODEL') and hasattr(server_module, 'TOKENIZER'):
            model = server_module.MODEL
            tokenizer = server_module.TOKENIZER
        
        # If model or tokenizer not available, load them
        ...
        # Generate output with sampling (for temperature)
        from models.demos.wormhole.ministral8b.tt.mistral_common import sample
        output_ids = []
        
        for _ in range(max_seq_len):
            # Get next token
            next_token = sample([input_ids + output_ids], model, temperature=temperature)[0]
            
            # Stop if we get EOS token
        ...
        logger.info(f"Generated response: {generated_text[:50]}{'...' if len(generated_text) > 50 else ''}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run Ministral-8B-Instruct-2410 demo with prefill")
    parser.add_argument("--device", type=str, default="tt", help="Device to run on")
    ...
    parser.add_argument("--n300", action="store_true", help="Running on N300 (vs N150)")
    parser.add_argument("--num-batches", type=int, default=1, help="Number of batches to run")
    parser.add_argument("--print-to-file", action="store_true", help="Print output to file")
    parser.add_argument("--input-prompt", type=str, default="", help="Single input prompt")

    args = parser.parse_args()

    device = ttnn.device(args.device)

    # Set up input prompt
    if args.input_prompt:
        input_prompts = [args.input_prompt]
    else:
        if args.instruct:
            input_prompts = "models/demos/wormhole/ministral8b/demo/input_data_questions_prefill_128.json"
        else:
            input_prompts = "models/demos/wormhole/ministral8b/demo/input_data_prefill_128.json"
